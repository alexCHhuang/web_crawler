...
def RANDOMGETUSR():
	while True:
		tmpuser = str(random.randint(1000001, 78788888))
		tc = urllib2.urlopen('http://api.douban.com/people/'+tmpuser)
		contents=tc.read()
		tc.close()
		dom = parseString(contents)
		xmlTag = dom.getElementsByTagName('db:uid')[0].toxml()
...
f = open(filename,'w')
db = MySQLdb.connect(host="140.109.21.248", port=3306, user="admin", passwd="sinica123456",db="douban")
cursor = db.cursor()
friend = 0
startIndex = 1
...
# set global default timeout is 30 seconds
